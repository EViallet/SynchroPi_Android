...
public class BluetoothHandler {
                    ...
                    if (valid) {
                        ...
                        for(String cmd : cmds) {
                            ...
                            if(cmd.contains("M"))
                                ...
                                activity.macConnected(Integer.decode(cmd.replace(SEP_MAC, "").replace("C", "").substring(0,1)),cmd.replace(SEP_MAC, "").replace("C", "").substring(0, 17));
    ...
    private boolean isComplete(String str) {
        return count(str,SEP_PACKETS)>0&&(count(str,SEP_DBG)>=2||(count(str,SEP_ID)>=2&&(count(str,SEP_BOOL)>=2||count(str,SEP_INT)>=2)||count(str,SEP_STR)>=2));
    }
    ...
    public void initBluetooth() {
        ...
        while(!isConnected&&currentIndex<activity.getMacs().size()) {
            try {
                ...
                Log.d(TAG,"Couldn't connect to "+activity.getMacs().get(currentIndex).ad,e);
                currentIndex++;
    ...
    public void initBluetooth(String mac) {
        try {
            ...
            Log.d(TAG,"Couldn't connect to "+mac,e);
        }
        if(!isConnected)
            bluetoothHandler.postDelayed(new Runnable() {
                @Override
